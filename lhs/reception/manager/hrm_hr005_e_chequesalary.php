...
<b><h3 class="panel-title text-center"><?php echo "Cheque Salary Generated FROM [".date('d/M/y', strtotime($xFromDate))."]TO [".date('d/M/y', strtotime($xToDate))."] As On ". date("d/M/y h:i:sa"); ?></h3></b>
</div>
...
$xQry="SELECT SUM( 
STATUS ) AS 
status , e.empname, e.empbasicsalary AS basicsalary,e.empepf,e.empesi,a.departmentno as departmentno,e.txno as empno
FROM  `attendence` as a , employeedetails AS e
WHERE date >= '$xFromDate' AND date<= '$xToDate'
AND e.txno = empno and (e.empdol>= '$xFromDate' or e.empstatus='ACTIVE') and emppaymentmode='CHEQUE' $xQryFilter
GROUP BY empno
...
$xTotalSalary=($xBasicSalary+$xDeductions)-($row['empesi']+$row['empepf']);
    ...
    echo '<td align=right>' . money_format("%!n", round($row['empepf'])). '</td>'; 
    echo'<td align=right>' .money_format("%!n", round($row['empesi'])). ' </td>';
...
$xGrandEsi+=$row['empesi'];
 ...
 echo '<td align=right>' . money_format("%!n", round($xGrandEpf)). '</td>'; 
 echo'<td align=right>' .money_format("%!n", round( $xGrandEsi)). ' </td>'; 
...
$xQry="SELECT SUM( STATUS ) AS status , e.empname, e.empbasicsalary AS basicsalary,e.empepf,e.empesi,a.departmentno as departmentno,empotherincentive FROM `attendence` as a , employeedetails AS e WHERE date >= '$xFromDate' AND date<= '$xToDate' AND e.txno =$xEmpNo and empno=$xEmpNo and (e.empdol>= '$xFromDate' or e.empstatus='ACTIVE') and emppaymentmode='CHEQUE' order by departmentno,empname";
$result2=mysql_query($xQry);
echo '<b>'; 
