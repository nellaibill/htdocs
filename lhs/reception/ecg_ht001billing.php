...
<select class="form-control"  value="" autofocus="autofocus" name="f_testtypeno" onchange="return onchangetesttype(this.value);" onfocus="return onchangetesttype(this.value);" disabled>
           <? }  ?>
                                                      <?php
                                                        $result = mysql_query("SELECT *  FROM m_testtype where testno in(1,2)");
                                                        while($row = mysql_fetch_array($result))
                                                        {
                                                      ?>
...
<div class="panel panel-info">
  ...
  <div class="panel-heading  text-center"><h3 class="panel-title">ECG-XRAY REPORT AS ON <?php echo date('d/F/Y', strtotime($GLOBALS ['xDate'])); ?></h3></div>
<table class="table">
      <thead>
        <tr>
  <th width="5%">S.NO</th>
...
if ( isset( $_GET['ecgtype'] ))
{
 ...
 {
$xQry="SELECT *  from t_ecgxraybilling where date='$xDate'  order by  txno"; 
 }
