...
<body>
<form action="viewexpensesbetweendateandproduct.php" method="post">
<div class="col-xs-3">
<select id="sc" name="productname" class="form-control">
    ...
    <option value="electrical">ELECTRICAL</option>
        <option value="doctorsalary">DOCTOR SALARY</option>
...
<div class="panel panel-info">
  ...
  <div class="panel-heading  text-center"><h3 class="panel-title">VIEW EXPENSES AS ON  <?php echo date('d/M/Y',strtotime($GLOBALS ['xFromDate'])) ?> TO <?php echo date('d/M/Y',strtotime( $GLOBALS ['xToDate']))?> GENERATED ON <?php echo date('d/M/Y h:i:s',strtotime( $GLOBALS ['xCurrentDate']))?></h3>
</div>
...
$query2="SELECT date , ".$_POST['productname']." as name ,$product1 as name1,$product2 as name2  from expenses WHERE date >= '".$xFromDate."' 
		 AND date<= '".$xToDate."' union all select 'GRAND-TOTAL',sum( ".$_POST['productname']."),'',''  from expenses 
...
while ($row = mysql_fetch_array($result2)) {
    echo '<tr>';
    echo '<td>' . $row['date']  . '</td>';
    echo '<td>' .  fn_RupeeFormat($row['name']) . '</td>';
