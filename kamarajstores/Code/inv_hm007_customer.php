...
if (isset ( $_GET ['account_ledger_id'] ) && ! empty ( $_GET ['account_ledger_id'] )) {
	...
	if ($_GET ['xmode'] == 'edit') {
		...
		if (! $result) {
			...
			header ( 'Location: inv_hm007_customer.php' );
		}
	}
} 
	...
	if ($count > 0) {
		while ( $row = mysql_fetch_array ( $result ) ) {
			...
			$GLOBALS ['xCreditLimit']  =$row ['credit_limit'];
                        $GLOBALS ['xOpeningBalance'] =$row ['opening_balance'];
		}
	}
}
...
function DataProcess($mode) {
	...
	$xCreditLimit = $_POST ['f_credit_limit'];
        $xOpeningBalance = $_POST ['f_opening_balance'];
	$xQry = "";
	$xMsg = "";
	if ($mode == 'S') {
		$xQry = "INSERT INTO account_ledger
		(account_ledger_id,ledger_name,ledger_alias_name,
		ledger_undergroup_no,ledger_address,ledger_mobile_no,ledger_unique_no,credit_limit,opening_balance)
		VALUES($xaccount_ledger_id,'$xLedger_Name','$xLedger_Alias_Name',$xLedger_Under_Group_No,
		'$xLedger_Address','$xLedger_MobileNo','$xLedger_Unique_No',$xCreditLimit,$xOpeningBalance)";
		...
		credit_limit=$xCreditLimit,opening_balance=$xOpeningBalance
...
<?php
$result = mysql_query ( "SELECT *  FROM account_group where account_group_id=5 order by account_group_name" );
while ( $row = mysql_fetch_array ( $result ) ) {
	?>
...
?>
	...
	<div class="col-xs-2" style="display: none;">
    	...
    	<div class="col-xs-2" style="display: none;">
				 ...
				 <label>Opening Balance:</label>	<input type="text" name="f_opening_balance"  class="form-control"
				value="<?php echo $GLOBALS ['xOpeningBalance']; ?>" />
		</div>
</div>
...
$xSlNo = 0;
$xTotalAmount = 0;
$xQry = "SELECT *  from account_ledger  where ledger_undergroup_no=5 order by account_ledger_id";
$result2 = mysql_query ( $xQry );
$rowCount = mysql_num_rows ( $result2 );
echo '</br>';

while ( $row = mysql_fetch_array ( $result2 ) ) {
	echo '<td width=20%>' . $row ['account_ledger_id'] . '</td>';
		echo '<td width=20%>' . $row ['ledger_name'] . '</td>';
	echo '<td width=20%>' . $row ['ledger_address'] . '</td>';
	echo '<td width=20%>' . $row ['ledger_mobile_no'] . '</td>';
	echo '<td width=20%>' . $row ['ledger_unique_no'] . '</td>';
	
	fn_FindAccountGroupName($row['ledger_undergroup_no']);
	echo '<td width=20%>' .$GLOBALS ['xAccountGroupName'] . '</td>';
        	
	?>
	<td><a
								href="inv_hm007_customer.php<?php echo '?account_ledger_id='.$row['account_ledger_id']. '&xmode=edit';  ?>"
								onclick="return confirm_edit()"> <img src="images/edit.png"
									style="width: 30px; height: 30px; border: 0">
							</a></td>
							<!--
							<td><a
								href="inv_hm007_customer.php<?php echo '?account_ledger_id='.$row['account_ledger_id']. '&xmode=delete';  ?>"
