...
<?php
$result = mysql_query ( "SELECT *  FROM account_group where account_group_id=4 order by account_group_name" );
while ( $row = mysql_fetch_array ( $result ) ) {
	?>
...
$xSlNo = 0;
$xTotalAmount = 0;
$xQry = "SELECT *  from account_ledger  where ledger_undergroup_no=4 order by ledger_name";
...
while ( $row = mysql_fetch_array ( $result2 ) ) {
	...
	?>
	<td><a
								href="inv_hm001supplier.php<?php echo '?account_ledger_id='.$row['account_ledger_id']. '&xmode=edit';  ?>"
								onclick="return confirm_edit()"> <img src="images/edit.png"
									style="width: 30px; height: 30px; border: 0">
							</a></td>
							<!--
							<td><a
								href="inv_hm001supplier.php<?php echo '?account_ledger_id='.$row['account_ledger_id']. '&xmode=delete';  ?>"
