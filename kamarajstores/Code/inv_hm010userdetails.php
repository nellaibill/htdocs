...
if (isset ( $_GET ['userno'] ) && ! empty ( $_GET ['userno'] )) {
	...
	if ($_GET ['xmode'] == 'edit') {
		...
		mysql_query ( $xQry );
		header ( 'Location: inv_hm010userdetails.php' );
	}
} else {
	GetMaxIdNo ();
}
...
while ( $row = mysql_fetch_array ( $result2 ) ) {
	...
	?>
<td><a
									href="inv_hm010userdetails.php<?php echo '?userno='.$row['userno'] . '&xmode=edit'; ?>"
									onclick="return confirm_edit()"> <img src="images/edit.png"
										alt="HTML tutorial"
										style="width: 30px; height: 30px; border: 0">
								</a></td>
								<td><a
									href="inv_hm010userdetails.php<?php echo '?userno='.$row['userno']. '&xmode=delete';  ?>"
