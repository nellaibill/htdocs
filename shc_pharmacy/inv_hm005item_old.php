...
function DataProcess($mode) {
    ...
    if ($mode == 'S') {
		...
		itemdescription,hsncode,packno,packdescription,gst,rackname,rowname,minstock,barcode,
                color,size,originalprice,mrp,disamount,supplierno
                )  
		VALUES 
		($xItemNo,$xStockPointNo,$xItemCategoryNo,
		$xItemGroupNo,0,
		'$xItemName','$xItemDescription','$xHSNCode',
		$xPackNo,'$xPackDescription','$xGst','$xRackName','$xRowName',$xMinStock,'$xBarCode','$xItemColor','$xItemSize',$xItemOriginalPrice,$xItemMrp,$xItemDisAmount,$xItemSupplierNo)";
       // echo $xQry;
		...
		packno=$xPackNo,packdescription='$xPackDescription',
                rackname='$xRackName',rowname='$xRowName',minstock=$xMinStock,barcode='$xBarCode',color='$xItemColor',size='$xItemSize',originalprice=$xItemOriginalPrice,mrp=$xItemMrp,disamount=$xItemDisAmount,supplierno=$xItemSupplierNo	WHERE itemno=$xItemNo";
       // echo $xQry;
        $xMsg = "Updated";
        $retval = mysql_query($xQry) or die(mysql_error());
        if (!$retval) {
