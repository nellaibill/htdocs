...
while ( $row = mysql_fetch_array ( $result ) ) { // Creates a loop to loop through results
	...
	$xSalesPerson_Id = $row ['salesperson_id'];
        	if($xSalesPerson_Id=="murugan")
	{
		$xSalesPerson_Id="Kowsi";
	}
	else
	{	
			$xSalesPerson_Id = $row ['salesperson_id'];
        ...
        //fn_FindLoginDetails($xServiceCharges);
	//fn_FindSalesPersonDetails($xSalesPerson_Id);
...
<body onload="window.print(); setTimeout(window.close, 0);">
	<div id="divToPrint">
				...
				<td style='font-size: 30pt; font-family:Arial' align=center > <img src=images/logo.jpg
		width=70px height=60px>" . $GLOBALS ['xCompanyTitle'] . " </td>
				...
				echo "<tr>
						...
						<td align=left width=12.5% > Print Date </td>
						<td width=12.5%>" . date("d/m/Y h:i:sa") . " </td>
				</tr>";
				echo "<tr>
						<td width=75% colspan=3 align=left > Dr Name - R.SRINIVASAN M.D,D.N.B(Cardio) </td>
		...
		echo "</table>";
				...
				while ( $row = mysql_fetch_array ( $result ) ) { // Creates a loop to loop through results
					...
					if($xCount<=1)
					{
					echo "Bill Date & Time  : ". $row ['updatedason'];
					}
					...
					echo "<tr class=hide_bottom> 
						...
						<td align=right width=10%> Rs." . $row ['unitrate'] . "</td>
						<td align=right width=10%>" . $xGst . " </td>
						<td align=right width=10%> Rs " . $xAmount . "  </td>
 </tr>";
					$xCount += 1;
			...
			/* 	for($i = $xCount; $xCount <= 10; $xCount ++) {
				...
				echo "<tr><td align=center  width=10% colspan=2>SGST</td><td width=10%> " . round ( $xFivePercentage, 2 ) . "  </td><td width=10%> " . round ( $xTwelvePercentage, 2 ) . "</td><td width=10%>" . round ( $xEighteenPercentage, 2 ) . " </td><td width=10%> " . round ( $xTwentyEightPercentage, 2 ) . " </td><td></td><td></td></tr>";
				echo "<tr><td align=center  width=60% colspan=6>" . ucwords ( convert_number_to_words ( $xBeforeRoundOffNetTotal ) ) . " Rupees Only </td><td width=20% align=right> NetAmount </td><td width=20% align=right> " . number_format ( $xBeforeRoundOffNetTotal, 2 ) . " </td></tr>";
				echo "<tr ><td width=60% class=sales_footer colspan=6>
				Total Rupees ". number_format ( $xBeforeRoundOffNetTotal, 2 ). " 
				 </td><td width=40% align=left> Bill By  " .$xSalesPerson_Id. " </td></tr>";
