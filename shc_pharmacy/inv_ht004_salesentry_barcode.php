...
if (isset($_GET ['salesmode']) && !empty($_GET ['salesmode'])) {
    GetMaxIdNo();
}
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function LoadBarcode() {
                var xBarCode = document.getElementById("f_barcode").value;
                document.getElementById('f_itemno').value = "";
                ...
                xmlhttp.onreadystatechange = function () {
                    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                        ...
                        document.getElementById("f_itemno").value = a[0];
                        document.getElementById("f_gst").value = a[1];
                        document.getElementById("f_taxincluded").value = a[2];
                        document.getElementById("f_purchasecode").innerHTML = a[3];
                    }
                }
                xmlhttp.open("GET", "getbarcode.php?barcode=" + xBarCode, true);
                xmlhttp.send();
            }
            function GetBatch() {
    ...
    <body onLoad="document.salesentryform.f_itemno.focus()">
        ...
        <div class="col-xs-2">
                                ...
                                <label>BarCode::</label> <input type="text" class="form-control"
                                                                name="f_barcode" id="f_barcode" 
                                                                onchange="LoadBarcode();"> 
                            </div>
                            ...
                            <div class="col-xs-6" >
                                <label>Item Name:</label> <select class="form-control"
                                    ...
                                    <?php
                                    $result = mysql_query("SELECT *  FROM m_item ");
                                    while ($row = mysql_fetch_array($result)) {
                                        ?>
    ...
    while ($row = mysql_fetch_array($result2)) {
        ...
        ?>
                            <tr class='clickable-row'
                                data-href="inv_ht004salesentry.php<?php echo '?txno=' . $row['txno'] . '&xmode=edit'; ?>">
        <?php
        echo '<td>' . $xSlNo . '</td>';
