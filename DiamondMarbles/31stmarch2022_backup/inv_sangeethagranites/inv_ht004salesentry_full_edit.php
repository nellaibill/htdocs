...
if (isset ( $_GET ['passsalesinvoiceno'] ) && ! empty ( $_GET ['passsalesinvoiceno'] )) {
	...
	fn_DataFetchSalesDetails ( $xPassSalesInvoiceNo );
}
if (isset ( $_GET ['txno'] ) && ! empty ( $_GET ['txno'] )) {
	...
	if ($_GET ['xmode'] == 'edit') {
		...
		header ( 'Location: inv_ht004salesentry_full_edit.php' );
	}
	...
	$xQ3 = mysql_query ( "update config_inventory set
			sales_invoice_no=0" ) or die ( mysql_error () );
	if ($xQ1 and $xQ2 and $xQ3) {
		mysql_query ( "COMMIT" );
...
function fn_DataFetchSalesDetails($xSalesInvoiceNo) {
	...
	if ($count > 0) {
		while ( $row = mysql_fetch_array ( $result ) ) {
			...
			$GLOBALS ['xCustomerNoforEdit'] = $row ['customerno'];
...
function GetMaxIdNo() {
	getconfig_inventoryvalues ();
	$xSalesInvoiceNo = $GLOBALS ['xConfigSalesInvoiceNo'];
	GetMaxTxNo ();
	fn_DataFetchSalesDetails ( $xSalesInvoiceNo );
}
/*
 * function GetMaxIdNo() {
 ...
 * while ( $row = mysql_fetch_array ( $result ) ) {
 * $GLOBALS ['xConfigSalesInvoiceNo'] = $row ['salesinvoiceno'];
 * $xSalesInvoiceNo = $GLOBALS ['xConfigSalesInvoiceNo'] + 1;
 * }
 * GetMaxTxNo ();
 * }
 */
...
if ($GLOBALS ['xConfigSales_InvoiceNo'] == 'Yes') {
	...
	?>
		...
		<?php }?>
						...
						value="<?php echo  $GLOBALS ['xConfigSalesInvoiceNo']; ?>"
						readonly>
				</div>
				...
				<div class="col-xs-6">
					<label>Item Name:</label> <select class="form-control"
						name="f_itemno" id="f_itemno" onblur="GetBatch();">
	...
	echo "<div class=col-xs-2>";
		...
		<?php }?>
						...
						<label>Disc %:</label> <input type="text" class="form-control"
								name="f_discountpercentage" value="0" id="f_discountpercentage">
						</div>
...
<?php
	...
	while ( $row = mysql_fetch_array ( $result2 ) ) {
		...
		echo '<td align=right>' . $xTotalBeforeDiscount . '</td>';
		...
		?>
<td><a
							href="inv_ht004salesentry_full_edit.php<?php echo '?txno='.$row['txno']. '&xmode=delete &passqty='.$row['qty']. ' &passitemno='.$row['itemno'] . ' &passbatch='.$row['batchid'] ?>"
...
<?php
	...
	echo '<td colspan="1" align="right" > ' . $xTotalAmount . '</td>';
	echo '</tr>';
...
<option value="<?php echo $row['account_ledger_id']; ?>"
							<?php
		if ($row ['account_ledger_id'] == $GLOBALS ['xCustomerNoforEdit']) {
			echo 'selected="selected"';
		}
		?>>
	...
	?>
				...
				<div class="col-xs-2">
					<label>Mode of Payment</label> <select class="form-control"
						...
						<option value="Cash"
							<?php if($GLOBALS ['xModeofPayment']=="Cash") echo 'selected="selected"'; ?>>Cash</option>
						<option value="Credit"
							<?php if( $GLOBALS ['xModeofPayment']=="Credit") echo 'selected="selected"'; ?>>Credit</option>
						<option value="Cheque"
							<?php if( $GLOBALS ['xModeofPayment']=="Cheque") echo 'selected="selected"'; ?>>Cheque</option>
					</select>
				</div>
...
<?php
	...
	if ($GLOBALS ['xConfigVehicleNo'] == 'Yes') {
		...
		<?php }?>
						...
						name="f_vehicleno" value="<?php echo $GLOBALS ['xVehicleNo'];?>">
				</div>
