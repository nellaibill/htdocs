...
</head><form action="viewexpensesbetweendate.php" method="post">
<body>

<div id="divToPrint" >
<div class="panel panel-info">
  <!-- Default panel contents -->
  <div class="panel-heading  text-center"><h3 class="panel-title">VIEW EXPENSES AS ON  <?php echo date('d/M/Y',strtotime($GLOBALS ['xFromDate'])) ?> TO <?php echo date('d/M/Y',strtotime( $GLOBALS ['xToDate']))?></h3></div>
...
FROM expenses WHERE date >= '$xFromDate' AND date<= '$xToDate'
UNION ALL 
SELECT  'GRAND-TOTAL', SUM( salary ) , SUM( esi ) , SUM( pf ) , SUM( eb ) , SUM( telephone ) , SUM( hspinstmaint ) , SUM( interdeptexp ) , SUM( hsptservice ) , SUM( ac ) , SUM( plumber ) , SUM( petrol ), SUM( medical),sum(cleaning),sum(dobby),sum(pharmacy),sum(gas),
sum(compservice),sum(incentive),sum(leaveincentive),sum(insurance),sum(civil),
sum(electrical),sum(doctorsalary),sum(lss),sum(others),SUM( salary + esi + pf + eb + telephone + hspinstmaint + interdeptexp +  hsptservice + ac + plumber + petrol+medical+cleaning+dobby+pharmacy+gas+compservice+incentive+leaveincentive+insurance+civil+electrical+doctorsalary+lss+others) 
FROM expenses WHERE date >= '$xFromDate' AND date<= '$xToDate' order by date") or die(mysql_error());
...
{
?>
<td><a href="editexpenses.php<?php echo '?date='.$expense_rows['date']; ?>" onclick="return confirm_edit()" ><img src="images/edit.png" alt="HTML tutorial" style="width:30px;height:30px;border:0"></a></td>
<td><a href="deleteexpenses.php<?php echo '?date='.$expense_rows['date']; ?>" onclick="return confirm_delete()" ><img src="images/delete.png" alt="HTML tutorial" style="width:30px;height:30px;border:0"></a></td>
<?php
}
else
{
