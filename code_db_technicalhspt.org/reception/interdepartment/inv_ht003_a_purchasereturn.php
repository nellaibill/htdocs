...
if ( isset( $_GET['txno'] ) && !empty( $_GET['txno'] ) )
{
  ...
  if($_GET['xmode']=='edit')
   ...
   else 
   {
         $xOldQty=$_GET['qty'];
         $xItemNo=$_GET['itemno'];
         $xQry = "DELETE FROM inv_purchasereturn WHERE txno= $no";
         ...
         header('Location: inv_ht003_a_purchasereturn.php'); 
   }
}
...
function GetMaxIdNo() 
   {
	$result = mysql_query ("SELECT  CASE WHEN max(txno)IS NULL OR max(txno)= '' THEN '1' 
                          ELSE max(txno)+1 END AS txno FROM  inv_purchasereturn") or die ( mysql_error () );
	while ( $row = mysql_fetch_array ( $result ) ) 
            {$GLOBALS ['xTxNo'] = $row ['txno'];}
   }
function DataFetch($xTxNo) {
    $result = mysql_query ( "SELECT *  FROM inv_purchasereturn where txno=$xTxNo" ) or die ( mysql_error () );
...
if ($mode == 'S') 
{
$xQry = "INSERT INTO inv_purchasereturn (txno,itemno,returnqty,returndetails,createdason,updatedason,loggeduser) 
                VALUES ($xTxNo,$xItemNo,$xReturnQty,'$xReturnDetails','$xCurrentDate','$xCurrentDate','$xCurrentUser')";
...
$xQry = "UPDATE inv_purchasereturn set 
...
?>
	...
	function validateForm() 
	 ...
	 var xQty= document.forms["purchasereturnform"]["f_returnqty"].value;
     	 if (xQty== "") 
	   {
		alert("Enter Qty");
		document.purchasereturnform.f_returnqty.focus();
		return false;
	   }
	...
	<body onload='document.purchasereturnform.f_returnqty.focus()'> 
	<form class="form" name="purchasereturnform" action="<?php echo $_SERVER['PHP_SELF']; ?>"method="post">
...
?>
	...
	<input type="number" class="form-control" name="f_returnqty" value="<?php echo $GLOBALS ['xReturnQty']; ?>" style="text-align:right;" >
	</div>


	<div class="col-xs-4">
	<label>Return Details:</label>
	<input type="text" class="form-control" name="f_returndetails" value="<?php echo $GLOBALS ['xReturnDetails']; ?>">
	</div>
...
<div class="panel panel-info">
  ...
  <div class="panel-heading  text-center"><b><h3 class="panel-title text-center"><?php echo "Purchase Return As On ". date("d/M/y h:i:sa"); ?></h3></b></div>
<table class="table">
      <thead>
        <tr>
           <th width="5%">S.No</th>
...
$xQry='';
$xSlNo=0;
$xQry="SELECT *  from inv_purchasereturn";
...
?>
<td><a href="inv_ht003_a_purchasereturn.php<?php echo '?txno='.$row['txno'] . '&xmode=edit'; ?>"  onclick="return confirm_edit()">
  <img src="../images/edit.png" alt="HTML tutorial" style="width:30px;height:30px;border:0">
</a>
</td>
<td><a href="inv_ht003_a_purchasereturn.php<?php echo '?txno='.$row['txno'].'&qty='.$row['returnqty']. '&itemno='.$row['itemno']. '&xmode=delete';  ?>"  onclick="return confirm_delete()">
